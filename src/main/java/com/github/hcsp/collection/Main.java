...
public class Main {
  ...
  public static Map<String, List<User>> collect(List<User> users) {
    HashMap<String, List<User>> group = new HashMap<String, List<User>>();
    for (int i = 0; i < users.size(); i++) {
      String department = users.get(i).getDepartment();
      if (!group.containsKey(department)) {
        group.put(department, new ArrayList<>());
      }
      group.get(department).add(users.get(i));
    }
    for (List list : group.values()) {
