...
public class Main {
    ...
    public static Map<String, List<User>> collect(List<User> users) {
        ...
        for (String department : departmentSet) {
            List<User> filterUserListByDepartment = users.stream().filter(user -> user.getDepartment().equals(department)).collect(Collectors.toList());
            map.put(department, sort(filterUserListByDepartment));
        }
        return map;
    }
