...
public class Main {
    ...
    public static Map<String, List<User>> collect(List<User> users) {
        Map<String, List<User>> userDepartmentMap = new HashMap<>();
        for (User user : users) {
            String userDep = user.getDepartment();
            if (userDepartmentMap.get(userDep) == null) {
                List<User> userTemp = new ArrayList<>();
                userTemp.add(user);
                userDepartmentMap.put(userDep, userTemp);
            } else {
                userDepartmentMap.get(userDep).add(user);
            }
        }
        for (Map.Entry<String, List<User>> entry : userDepartmentMap.entrySet()) {
