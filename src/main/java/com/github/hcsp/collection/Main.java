...
public class Main {
    ...
    public static Map<String, List<User>> collect(List<User> users) {
        Map<String, List<User>> result = new HashMap<>(users.size());
        for (User user : users
        ) {
            String department = user.getDepartment();
            result.put(department, null);
        }
        Set<String> departmentSet = result.keySet();
        for (String department : departmentSet
        ) {
            List<User> usersCachedByDepartment = new ArrayList<>();
            for (User user : users
            ) {
                if (user.getDepartment().equals(department)) {
                    usersCachedByDepartment.add(user);
                }
            }
            Collections.sort(usersCachedByDepartment);
            result.put(department, usersCachedByDepartment);
        }
        return result;
