...
public class Main {
    ...
    public static Map<String, List<User>> collect(List<User> users) {
        //https://stackoverflow.com/questions/48563867/add-to-list-value-on-hashmap
        Map<String, List<User>> hashMap = new HashMap<>();
        for (User user : users) {
            hashMap.computeIfAbsent(user.getDepartment(), key -> new ArrayList<>()).add(user);
        }
        for (List<User> list : hashMap.values()) {
