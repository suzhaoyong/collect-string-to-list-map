...
public class Main {
    ...
    public static Map<String, List<User>> collect(List<User> users) {
        ...
        for (User user : users) {
            ...
            if (result.containsKey(departmentName)) {
                result.get(departmentName).add(user);
            } else {
                result.put(departmentName, new ArrayList<>(Collections.singletonList(user)));
            }
        }
        Set<String> strings = result.keySet();
        for (String string : strings) {
            result.get(string).sort(new Comparator<User>() {
                @Override
