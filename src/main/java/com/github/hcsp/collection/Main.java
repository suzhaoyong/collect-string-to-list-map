...
public class Main {
    ...
    public static Map<String, List<User>> collect(List<User> users) {
        Map<String, List<User>> mapping = new HashMap<>();
        for (User user : users) {
            if (!mapping.containsKey(user.getDepartment())) {
                mapping.put(user.getDepartment(), new ArrayList<>());
            }
            mapping.get(user.getDepartment()).add(user);
        }
        for (List<User> value : mapping.values()) {
